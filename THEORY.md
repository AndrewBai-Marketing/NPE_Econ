...
\mathcal N\!\left(z;m_k(u),\mathrm{diag}(\exp\ell_k(u))\right).
```
...
\mathrm{KL}\!\left(p_X\,\Vert\,Q_X\right).
```
...
```math
\mathbb E_X\mathrm{KL}(p_X\Vert\widehat Q_{n,X})
...
```math
\mathbb E_Y\mathrm{KL}\!\left(p(\Theta\mid Y)\Vert Q_{S(Y)}\right)
=I(\Theta;Y\mid X)
+\mathbb E_X\mathrm{KL}(p_X\Vert Q_X).
```
...
```math
\mathrm{TV}(h_\#p_x,h_\#Q_x)
\leq\mathrm{TV}(p_x,Q_x)
\leq\sqrt{\tfrac12\mathrm{KL}(p_x\Vert Q_x)}.
```
...
$`\sqrt{\mathrm{Var}_{Q_x}(h)/R}`$. More draws reduce that sampling error,
...
\leq\mathbb E_X\mathrm{TV}(p_X,Q_X).
```
